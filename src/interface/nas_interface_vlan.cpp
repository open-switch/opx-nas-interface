...
t_std_error NAS_VLAN_INTERFACE::set_mtu(size_t mtu){
    ...
    if((rc = nas_os_vlan_set_member_port_mtu(og.get())) != STD_ERR_OK) {
        EV_LOGGING(INTERFACE,ERR,"NAS-VLAN","Failed to set mtu to %d for vlan sub interface %s",if_name.c_str());
        return rc;
    }
