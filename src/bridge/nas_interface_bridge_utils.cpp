...
t_std_error nas_bridge_create_vlan(const char *br_name, hal_vlan_id_t vlan_id, hal_ifindex_t *idx,  NAS_BRIDGE **bridge_obj)
{
    t_std_error rc = STD_ERR_OK;
    ...
    if ((rc = nas_bridge_utils_create_obj(br_name, BASE_IF_BRIDGE_MODE_1Q, *idx, (NAS_BRIDGE **)&dot1q_br_obj)) != STD_ERR_OK) {
...
t_std_error nas_bridge_utils_delete(const char *name)
{
    ...
    if ((rc = br_obj->nas_bridge_npu_delete()) != STD_ERR_OK) {
        EV_LOGGING(INTERFACE,ERR,"NAS-BRIDGE", " Failed to delete bridge in the NPU for %s", name);
        return rc;
    }
...
//     Get the complete member list
    ...
    //     Add the members back in to the vlan bridge memberlist
...
t_std_error nas_bridge_utils_add_remote_endpoint(const char *vxlan_intf_name,remote_endpoint_t & rem_ep) {
    ...
    if (tunnel_exist == true) {
        if ( (rc= dot1d_bridge->nas_bridge_set_flooding(&rem_ep)) != STD_ERR_OK) {
...
t_std_error nas_bridge_utils_update_remote_endpoint(const char *vxlan_intf_name,
    ...
    if(cur_rem_ep.mac_learn_mode != rem_ep.mac_learn_mode){
        if((rc = vxlan_obj->nas_interface_set_mac_learn_remote_endpt(&rem_ep))!=STD_ERR_OK){
            return rc;
        }
    }

    if(cur_rem_ep.flooding_enabled != rem_ep.flooding_enabled){
        NAS_BRIDGE *bridge_obj = nullptr;
...
t_std_error nas_bridge_utils_set_untagged_vlan(const char * bridge_name, hal_vlan_id_t vlan_id){
    ...
    std::string _br_name(bridge_name);
    NAS_BRIDGE *br_obj = nullptr;
    if (nas_bridge_map_obj_get(_br_name, &br_obj) != STD_ERR_OK) {
        EV_LOGGING(INTERFACE,ERR,"NAS-INT", " Bridge %s not present in the map ", _br_name);
        return rc;
    }

    NAS_DOT1D_BRIDGE * dot1d_br_obj = dynamic_cast<NAS_DOT1D_BRIDGE *>(br_obj);

    dot1d_br_obj->untagged_vlan_id = vlan_id;
    return STD_ERR_OK;
}
