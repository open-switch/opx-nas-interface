...
static cps_api_return_code_t nas_vlan_intf_cps_get(void * context, cps_api_get_params_t *param, size_t ix)
{
    ...
    cps_api_object_attr_t _name = cps_api_get_key_data(filt,IF_INTERFACES_INTERFACE_NAME);
    ...
    if ((_name == nullptr) && (_vlan_id == nullptr))  {
        /*  Get all */
        nas_fill_all_bridge_info(&param->list,INT_VLAN_MODEL);
        ...
        if (_name != nullptr) {
            ...
            std::string _br_name;
            if (nas_bridge_vlan_to_bridge_get(vid, _br_name) != STD_ERR_OK) {
...
static cps_api_return_code_t nas_cps_set_vlan(cps_api_object_t obj)
{
    ...
    //nas_bridge_utils_publish_vlan_intf_event(br_name, cps_api_oper_SET);
    return cps_api_ret_code_OK;
}
...
static cps_api_return_code_t nas_cps_create_vlan(cps_api_object_t obj)
{
    ...
    do {
        hal_ifindex_t idx;
        rc = cps_api_ret_code_ERR;
        if (nas_bridge_create_vlan(name, vlan_id, &idx) != STD_ERR_OK) {
            EV_LOGGING(INTERFACE, ERR, "NAS-Vlan", "Bridge creation failed %s", name);
            break;
        }

        if (!parent_bridge.empty()) {
            nas_bridge_utils_parent_bridge_set(name, parent_bridge);
        }
    ...
    nas_bridge_utils_publish_vlan_intf_event(name, cps_api_oper_CREATE);
...
static cps_api_return_code_t nas_cps_delete_vlan(cps_api_object_t obj)
{
    ...
    if( vlan_name_attr == nullptr) {
        hal_ifindex_t if_index =  cps_api_object_attr_data_u32(vlan_if_attr);
    ...
    nas_bridge_utils_publish_vlan_intf_event(br_name, cps_api_oper_DELETE);
    // TODO Same object can be published also
